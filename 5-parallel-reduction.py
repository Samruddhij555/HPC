...
__global__ void reduceAverage(int* input, float* output, int size) {
    ...
    // Write the result for this block to global memory
    if (tid == 0) {
        output[blockIdx.x] = sdata[0] / static_cast(size);
    }
}
int main() {
    // Input array
    ...
    reduceMin<<>>(d_input, d_output_min, array_size);
    reduceMax<<>>(d_input, d_output_max, array_size);
    reduceSum<<>>(d_input, d_output_sum, array_size);
    reduceAverage<<>>(d_input, d_output_avg, array_size);
